...
- **Helper changes during Pi reload** — Pi/Jiti can retain old native `.mjs` exports even when it reloads the extension entry point. Helpers are loaded through a small Node-native bootstrap with content-hashed module URLs, avoiding mixed revisions (the observed failure was `stopOwnedProcessTree is not a function`). Run `PI_JITI_MODULE=/path/to/pi/node_modules/jiti/lib/jiti.mjs node --test tests/*.test.mjs` to exercise the real loader as well as the ordinary tests.
